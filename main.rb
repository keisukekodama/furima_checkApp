...
def sign_up_retry
  ...
  @wait.until {@d.find_element(:id, 'password').displayed?}
  @d.find_element(:id, 'password').send_keys(@password)
  puts "◯パスワードが必須であるパスワードは、6文字以上での入力が必須であること"
...
def login_user1
  ...
  #１時的な措置、ログイン・ログアウトができるなら下記２つの要件達成しているという判断
  puts "◯ヘッダーのログアウトボタンをクリックすることで、ログアウトができること"
  puts "◯ログイン状態では、ヘッダーにユーザーのニックネーム/ログアウトボタンが表示されること"
...
def item_new_no_image
  ...
  if /商品の情報を入力/.match(@d.page_source)
    puts "◯商品画像を1枚つけることが必須であること(ActiveStorageを使用すること)" 
    else
    puts "☒商品画像を1枚つけることが必須ではない" 
    @wait.until {@d.find_element(:id,"item-name").displayed?}
  end
end
...
def item_new_price_uninput
  ...
  puts "◯入力に問題がある状態で出品ボタンが押されたら、出品ページに戻りエラーメッセージが表示されること"
  sleep 3
...
def item_edit
  ...
  puts "◯必要な情報を適切に入力すると、商品情報（商品画像・商品名・商品の状態など）を変更できること"
  puts "◯エラーメッセージの出力は、商品情報編集ページにて行うこと"
end
...
def login_user2_item_buy
  ...
  puts "◯ログイン状態の出品者のみ、「編集・削除ボタン」が表示されること"
  puts "◯ログイン状態の出品者だけが商品情報編集ページに遷移できること"
...
def login_user2_after_purchase_check1
  ...
  puts "必要な情報を適切に入力すると、商品の購入ができること"
  puts "入力に問題がある状態で購入ボタンが押されたら、購入ページに戻りエラーメッセージが表示されること"

  # order_url_coatは購入画面のURLでは？商品ページではないハズ
  @d.get(@order_url_coat)
  if /FURIMAが選ばれる3つの理由/ .match(@d.page_source)
    puts "◯ログイン状態のユーザーが、URLを直接入力して売却済み商品の商品購入ページへ遷移しようとすると、トップページに遷移すること"
  else
    puts "☒◯ログイン状態のユーザーが、URLを直接入力して売却済み商品の商品購入ページへ遷移しようとすると、トップページに遷移しない"
    # 手動でトップページに遷移させる理由は？？
...
def login_user2_item_new
  ...
  @d.find_element(:class,"sell-btn").click
  #出品後にトップページに遷移しないばあいの処理を加えたい(児玉)
end
...
def no_user_item_buy
  ...
  if /会員情報入力/ .match(@d.page_source)
    ...
    puts "◯ログアウト状態のユーザーは、URLを直接入力して商品購入ページに遷移しようとすると、商品の販売状況に関わらずログインページに遷移すること"
  else
    puts "☒ログアウト状態のユーザーは、URLを直接入力して商品購入ページに遷移しようとすると、商品の販売状況に関わらずログインページに遷移しない"
  ...
  @d.get(@order_url_glasses)
  if /FURIMAが選ばれる3つの理由/ .match(@d.page_source)
    puts "◯ログイン状態の出品者が、URLを直接入力して自身の出品した商品購入ページに遷移しようとすると、トップページに遷移すること"
  else
    puts "☒ログイン状態の出品者が、URLを直接入力して自身の出品した商品購入ページに遷移しようとすると、トップページに遷移しない"
    puts  "!手動でトップページに遷移するとプログラムが動きます。"
    @wait.until {@d.find_element(:class,"purchase-btn").displayed?}
  end
  ...
  @d.find_element(:class,"item-img-content").click 
  if /購入画面に進む/ .match(@d.page_source)
    puts "☒ログイン状態の出品者以外のユーザーでも、「購入画面に進むボタン」が表示される"
  else
    puts "◯ログイン状態の出品者以外のユーザーのみ、「購入画面に進むボタン」が表示されること" 
  end
  ...
  puts "上から、出品された日時が新しい順に表示されること"
  puts "ヘッダーの新規登録/ログインボタンをクリックすることで、各ページに遷移できること"
